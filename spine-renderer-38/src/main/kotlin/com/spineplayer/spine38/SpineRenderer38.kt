package com.spineplayer.spine38
...
/**
 * Spine 3.8 版本渲染器实现
 * 使用 spine-libgdx 3.8.99.1 runtime（通过 shadow plugin 重定位到 com.spineplayer.spine38）
 *
 * 与 4.2 的关键 API 差异：
...
class SpineRenderer38 : SpineRenderer {
    ...
    override fun load(skeletonFile: File, atlasFile: File): Boolean {
        return try {
            ...
            Gdx.app.log(TAG, "Loaded 3.8 model: ${skeletonFile.name}, animations: ${animationList.size}")
            true
        } catch (e: Exception) {
            Gdx.app.error(TAG, "Failed to load 3.8 skeleton: ${skeletonFile.name}", e)
            dispose()
            false
        }
    }
    ...
    companion object {
        private const val TAG = "SpineRenderer38"
    }
}
