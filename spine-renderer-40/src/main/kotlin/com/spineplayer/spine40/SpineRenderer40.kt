package com.spineplayer.spine40
...
/**
 * Spine 4.0 版本渲染器实现
 * 使用 spine-libgdx 4.0.18.1 runtime（通过 shadow plugin 重定位到 com.spineplayer.spine40）
 *
 * 与 4.2 的关键 API 差异：
 * - skeleton.updateWorldTransform() 无参数（Physics 是 4.2 才引入的）
 */
class SpineRenderer40 : SpineRenderer {
    override val supportedVersion = SpineVersion.V4_0
    ...
    /** 渲染自检是否已输出（首帧输出一次） */
    private var renderDiagnosticShown = false
    ...
    override fun load(skeletonFile: File, atlasFile: File): Boolean {
        return try {
            ...
            // atlas 的 pma 标记为 true 时，必须让渲染器按预乘 alpha 处理颜色与混合，
            // 否则 pma:true 纹理按非预乘渲染会出现颜色偏暗/黑边（atlas 第一页前的 "pma:true" 行）。
            val atlasPma = try {
                // atlas 头部 "pma:true" 时 TextureAtlas 内部没有直接暴露 pma 标记，这里按 atlas 文件内容判断
                atlasFile.readBytes().toString(Charsets.UTF_8).lines()
                    .firstOrNull { it.trim().startsWith("pma:") }
                    ?.trim()?.substringAfter(':')?.trim()?.equals("true", ignoreCase = true) == true
            } catch (e: Exception) { false }
            if (atlasPma) {
                skeletonRenderer?.setPremultipliedAlpha(true)
                Gdx.app.log(TAG, "atlas pma:true, SkeletonRenderer premultipliedAlpha enabled")
            }
            ...
            Gdx.app.log(TAG, "Loaded 4.0 model: ${skeletonFile.name}, animations: ${animationList.size}")
            true
        } catch (e: Exception) {
            Gdx.app.error(TAG, "Failed to load 4.0 skeleton: ${skeletonFile.name}", e)
            dispose()
            false
        }
    }
    ...
    override fun render(batch: PolygonSpriteBatch, x: Float, y: Float, scale: Float) {
        ...
        skel.scaleY = scale
        skel.updateWorldTransform()
        // 渲染自检（首帧）：确认有可见附件被绘制，若为 0 说明附件/纹理链路异常
        if (!renderDiagnosticShown) {
            renderDiagnosticShown = true
            val drawable = skel.drawOrder.count { it.attachment != null }
            val meshWithRegion = skel.drawOrder.count { slot ->
                val a = slot.attachment
                a is MeshAttachment && a.region != null
            }
            Gdx.app.log(TAG, "render diag: drawableSlots=$drawable meshWithRegion=$meshWithRegion pos=($x,$y) scale=$scale " +
                    "bounds=(${skeletonData?.width},${skeletonData?.height})")
        }
    ...
    companion object {
        private const val TAG = "SpineRenderer40"
    }
}
